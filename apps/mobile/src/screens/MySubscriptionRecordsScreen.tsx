...
export default function MySubscriptionRecordsScreen({ 
  lang = 'zh', 
  userInfo, 
  onClose, 
  ...
  const loadData = useCallback(async (isRefresh: boolean = false) => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        setSubscriptionRecords(formattedRecords);
      } else {
        setSubscriptionRecords([]);
      }
    } catch (error) {
      console.error('获取申购记录时发生异常:', error);

...
export default function MyRedemptionRecordsScreen({ 
  lang = 'zh', 
  userInfo, 
  onClose, 
  ...
  const loadData = useCallback(async (isRefresh: boolean = false) => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .eq('type', 'redemption') // 只获取赎回记录
        .order('created_at', { ascending: false });

      if (error) {
        console.error('获取赎回记录失败:', error);
        setRedemptionRecords([]);
        return;
      }
      ...
      console.error('获取赎回记录时发生异常:', error);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.functionButtons}>
        ...
        >
          <Text style={styles.functionButtonIcon}>📝</Text>
          <Text style={styles.functionButtonText}>{t.contractSigning}</Text>
        </TouchableOpacity>
      </View>
            ...
            redemptionRecords.map(item => (
              <View key={item.id} style={styles.recordCard}>
                ...
                <View style={styles.row}>
                  ...
                  <View style={styles.cell}>
                    <Text style={styles.cellLabel}>{t.shares}</Text>
